...
if [[ -z "$API_BASE" || -z "$ADMIN_UI_BASE" ]]; then
  echo ""
  echo "⚠️  API_BASE and/or ADMIN_UI_BASE not set."
  echo "   Example:"
...
def sub(key, val, text):
    ...
    if not re.search(pat, text):
        print(f"❌ Could not find key in config.js: {key}", file=sys.stderr)
        sys.exit(1)
...
echo ""
echo "✅ Owner Portal ready at: $ROOT"
echo "   Files:"
ls -1 "$ROOT" | sed 's/^/   - /'

echo ""
echo "▶ Run locally (no auth):"
echo "   cd \"$ROOT\" && PORT=$PORT node server.js"
echo ""
echo "🔒 Run with Basic Auth (recommended):"
